...
import LoginCredentials from "../../admin/components/LoginCredentials";
import DocumentUpload from "../../admin/components/DocumentUpload";
import { DEFAULT_PASSWORD } from "../staffPassword";
import {
  Alert,
  Button,
  Card,
 ...
 */
/**
 * Hiring somebody, in four steps — the same form wherever it is opened.
 *
 * It lived in the admin panel and HR had a shorter one of its own: two steps,
 * in a modal, asking for a subset of the same fields. Two forms writing to one
 * endpoint is one of them being wrong about what an employee record is, and
 * the shorter one was — HR is the department that actually collects the
 * Aadhaar card and the bank account.
 *
 * So there is one form, here, and the two panels differ only in what they hand
 * it: their own axios instance (through `useRecordForm`, built by
 * makeCrudHooks for that panel) and their own list of people somebody can
 * report to. Everything else — the steps, the validation, the paperwork
 * uploads, the login rule — is the same code for both, which is the only
 * arrangement in which they cannot drift apart again.
 */
export default function StaffForm({
  ...
  /**
   * The panel's record-form hook — `useRecordForm` from admin/hooks/crud or
   * hr/hooks/crud. Passed in rather than imported because it is the one thing
   * that decides which API this form reads and writes.
   */
  useRecordForm,
  /** Who "Reports to" may name. The panels fetch this differently. */
  reportsToOptions = [],
  /**
   * Whether an edit from this panel may change the password.
   *
   * HR's update route strips it, so on HR the box is not offered rather than
   * offered and quietly ignored. Creating is unaffected either way — that is
   * where the login is minted, and HR's create endpoint does set it.
   */
  canSetPasswordOnEdit = true,
  /**
   * Where a create goes, when it is not the panel's own staff endpoint.
   *
   * Hiring a candidate is what this exists for. The record it writes is an
   * employee, so the form is this form — but the write has to go through the
   * hire route, which also links the candidate to the new account, carries
   * their CV across and closes the vacancy they filled. A (body) => result
   * function, truthy on success, like `submit` itself. Creates only: editing
   * an existing record is always the panel's own endpoint.
   */
  saveRecord,
  /**
   * Values to start a create with, arriving after the first render because
   * the caller had to fetch them.
   */
  prefill,
  /** Overrides, where the form is doing something narrower than "add staff". */
  ...
  /**
   * Values the caller had to fetch, folded in once.
   *
   * Done during render rather than in an effect: an effect paints the empty
   * form first and the filled one a tick later, which flashes every field.
   * Once, because a second pass would undo whatever has been typed since.
   */
  const [prefilled, setPrefilled] = useState(false);
  if (prefill && !prefilled && !isEdit && !loading) {
    setPrefilled(true);
  ...
  const handleSubmit = async (e) => {
    ...
    const ok = saveRecord && !isEdit ? await saveRecord(body) : await submit(body);
    if (!ok) return;
  ...
  const heading = headingOverride || (isEdit ? `Edit ${title}` : `Add ${title}`);
  const blurb =
    blurbOverride ||
    (isEdit
          ...
          <div className="p-5">
            ...
            {step === 0 && (
              <BasicStep
                ...
                canSetPassword={!isEdit || canSetPasswordOnEdit}
