...
import { Eye, EyeOff, Users, CalendarOff, UserPlus, ClipboardCheck } from "lucide-react";

import api from "../api";
import { forgetHrAccess } from "./hooks/useHrAccess";

const HIGHLIGHTS = [
  { icon: CalendarOff, text: "Leave, from the request to the decision" },
  { icon: UserPlus, text: "Hiring, from first CV to their first login" },
  { icon: ClipboardCheck, text: "Attendance, documents and the monthly report" },
];
...
export default function HrLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post("/hr/login", form);
      ...
      <div className="relative hidden flex-col justify-between bg-[#0B0F1A] p-12 lg:flex">
        <div className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            ...
            <p className="text-[11px] uppercase tracking-widest text-slate-500">HR Panel</p>
          </div>
        </div>
        ...
        <div>
          ...
          <p className="mt-3 max-w-sm text-sm text-slate-400">
            Who works here, who is joining, and who is away — without asking anybody for a
            spreadsheet.
          </p>
        ...
        <div className="w-full max-w-sm">
          <div className="mb-8 flex items-center gap-3 lg:hidden">
            ...
            <div className="leading-tight">
              ...
              <p className="text-[11px] uppercase tracking-widest text-slate-400">HR Panel</p>
            </div>
          </div>

          <h1 className="text-2xl font-bold tracking-tight text-slate-900">HR sign in</h1>
          <p className="mt-1 text-sm text-slate-500">
            For the HR head and the HR managers they set up
          </p>
          ...
          <p className="mt-8 text-center text-[11px] leading-relaxed text-slate-400">
            The HR head's account is created by the administrator; HR managers are created by the
            HR head. There is no self sign-up.
          </p>
        </div>
      </div>
    </div>
