 ...
 *
 * These are the accounts the admin panel calls Operations Managers: the people
 * who run the projects, and whom the employees report to. They are staff, and
 * staff are HR's job, so HR opens the login here rather than asking an
 * administrator for one every time somebody is hired.
 *
 * The screen beside this one — Managers — is for department heads, a different
 * role with a different job, and HR Managers below it is for logins on this
 * panel. The server writes role:"operations_manager" on every save whatever
 * this form sends, so nothing here can create an administrator or a login for
 * another panel.
 *
 ...
 * record in the app, so an operations manager HR opens is as complete as one
 * an administrator opens. Editing does not ask again, because the requirement
 * is on creation only.
 */
...
/**
 * The paperwork is collected here, and does not hold the account up.
 *
 * The fields are on the form and are saved, but a missing scan does not refuse
 * the create — the server stopped demanding them for the reason the managers
 * screen records: a person starts on Monday and their PAN photo arrives on
 * Thursday, and blocking the login until then helped nobody. What is missing
 * shows on the person's own record, which is where somebody chasing it looks.
 */
 ...
 */
export default function OperationsManagers({ embedded = false }) {
  ...
  const canCreate = can("operations_managers", "create");
  const canEdit = can("operations_managers", "edit");
  const canDelete = can("operations_managers", "delete");
  ...
  useEffect(() => {
    ...
    Promise.all([
      hrApi.get("/hr/operations-managers", {
        params: { search: search || undefined, status: status || undefined },
      }),
      hrApi.get("/hr/operations-managers/summary"),
    ])
      .then(([list, summary]) => {
        if (!active) return;
      ...
      .catch((err) => {
        if (active) setError(err.response?.data?.message || "Could not load operations managers");
      })
  ...
  const openEdit = (row) => {
    setForm({
      ...
      email: row.email || "",
      phone: row.phone || "",
      designation: row.designation || "Operations Manager",
  ...
  const save = async (e) => {
    ...
    try {
      if (isNew) {
        ...
        await hrApi.post("/hr/operations-managers", body);
        ...
        await hrApi.put(`/hr/operations-managers/${editing._id}`, payload);
        setNotice("Operations manager updated");
        closeForm();
      }
      reload();
    } catch (err) {
  ...
  const toggleStatus = async (row) => {
    ...
    setBusy(true);
    try {
      await hrApi.put(`/hr/operations-managers/${row._id}`, {
   ...
   */
  const removeOperationsManager = async (row) => {
    setNotice("");
    setBusy(true);
    try {
      await hrApi.delete(`/hr/operations-managers/${row._id}`);
    ...
    },
    {
      ...
      render: (row) => (
        <div>
          ...
          <p className="text-xs text-slate-500">{row.designation || "Operations Manager"}</p>
        </div>
      ),
    },
    ...
    <div className="space-y-4">
      {!embedded && (
        <PageHeader
          ...
          subtitle="They run the projects and the employees report to them. They sign in at the operations manager panel."
        >
          {canCreate && (
            <Button onClick={openNew}>
              <Plus size={16} /> Add operations manager
            </Button>
          )}
        </PageHeader>
      )}
      ...
      <Card>
        <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 p-3">
          ...
          {embedded && canCreate && (
            <Button size="sm" className="ml-auto" onClick={openNew}>
              <Plus size={15} /> Add operations manager
            </Button>
          )}
        </div>
        ...
        <DataTable
          ...
          rows={rows}
          loading={loading}
          emptyTitle="No operations managers yet"
          emptyMessage="Add one and they will be able to sign in at the operations manager panel and pick up projects."
        />
      </Card>
      ...
      <Modal
        ...
        title={isNew ? "Add operations manager" : `Edit ${editing?.name || ""}`}
        subtitle={
          credentials
            ? undefined
            : isNew
              ...
              : "An operations manager with a login for the operations manager panel"
        }
        onClose={closeForm}
        size="lg"
        footer={
            ...
            <>
              ...
              {isNew && step === 0 ? (
                ...
                <Button onClick={save} disabled={saving}>
                  {saving ? "Saving…" : isNew ? "Create operations manager" : "Save changes"}
                </Button>
              )}
            </>
          )
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label="Department">
                    <Input
                      ...
                      onChange={change}
                      placeholder="Operations, Execution…"
                    />
                  </Field>
                </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={() =>
          confirming?.action === "delete"
            ? removeOperationsManager(confirming.row)
