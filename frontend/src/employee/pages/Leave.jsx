...
export default function EmployeeLeave() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const { data } = await employeeApi.get("/employee/leaves");
      setRows(data.items || []);
  ...
  const submit = async (e) => {
    ...
    setFormError("");
    try {
      const { data } = await employeeApi.post("/employee/leaves", {
        ...form,
      ...
      setApplying(false);
      setNotice(data.message || "Your request has been sent to HR");
      load();
    } catch (err) {
  ...
  const confirmWithdraw = async () => {
    ...
    setSaving(true);
    try {
      await employeeApi.put(`/employee/leaves/${withdrawing._id}/withdraw`);
      setWithdrawing(null);
